    ...
    return (
        ...
        >
            <img
                src={openLetter.src}
                alt="open letter"
                className={`relative w-full h-auto top-0 left-0 -translate-y-[8rem] sm:-translate-y-[9.2rem] md:-translate-y-[11.5rem] lg:-translate-y-[14.2rem] transition-opacity duration-300 ${isOpen ? "opacity-100" : "opacity-0"
                    }`}
            />
            <img
                src={closedLetter.src}
                alt="closed letter"
                className={`absolute w-full h-auto top-0 left-0 transition-opacity duration-300 ${isOpen ? "opacity-0" : "opacity-100"
                    }`}
            />
        </div>
    );
